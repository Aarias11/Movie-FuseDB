...
const slideRight = () => {
    ...
    useEffect (() => {
        ...
        axios
        .get(`https://api.themoviedb.org/3/tv/${id}/credits?api_key=c207799c1506767bae69bab29daa6238&language=en-US`)
        .then((res) => {
    ...
    <div id={'sliderOne'} className='flex sm:flex overflow-x-scroll gap-3 '>
    
        {
            cast.map(cast => (
              ...
              <img className=' w-[250px] border border-gray-500 shadow-lg shadow-gray-700 rounded-xl transform transition-all hover:scale-105 ' src={`https://image.tmdb.org/t/p/w500/${cast.profile_path}`} alt='tv_poster'/>
              </Link>
                ) : (
