...
function TopRated() {
    const url = 'https://api.themoviedb.org/3/movie/top_rated?api_key=c207799c1506767bae69bab29daa6238&language=en-US&page=1'
...
const slideRight = () => {
  ...
  return (
    <div className='w-full h-auto bg-[#1a1a1a] bg-gradient-to-r from-black w-min-full relative group'>
        <h1 className='text-2xl font-bold p-6 text-white'>Top Rated
        </h1>
        <div id={'sliderFour'} className='flex sm:flex overflow-x-scroll gap-3 '>
        
            {
                movies.map(movies => (
                    <div key={movies.id} className='relative w-[600px] p-3 flex flex-col '>
                    <Link to={`/movies/${movies.id}`}>
                    <img className=' w-[250px] border border-gray-500 shadow-lg shadow-gray-700 rounded-xl transform transition-all hover:scale-105 ' src={`https://image.tmdb.org/t/p/w500/${movies.poster_path}`} alt='movie_poster' />
                    </Link>
