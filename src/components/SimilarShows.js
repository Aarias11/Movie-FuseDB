...
function SimilarShows() {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `https://api.themoviedb.org/3/tv/${id}/similar?api_key=c207799c1506767bae69bab29daa6238&language=en-US&page=1`
      )
      .then((res) => {
        setSimilarShows(res.data.results);
  ...
  return (
    <div className="relative group">
      <h2 className="p-5 text-center bg-black text-white text-2xl">
        SIMILAR TITLES
      </h2>
      <div
        id={"sliderSimilar"}
        className="w-full h-[400px] bg-black text-white flex overflow-x-scroll gap-3"
      >
        {similarShows.map((similarShows) => (
          <div
            key={similarShows.id}
            className="relative w-[600px] p-3 flex flex-col  ">
            {similarShows.poster_path ? (
              <Link to={`/shows/${similarShows.id}`}>
                <img
                  ...
                  src={`https://image.tmdb.org/t/p/w500/${similarShows.poster_path}`} alt="tv_poster"
                />
              </Link>
            ) : (
              <div className="pt-2 filler-poster relative w-[200px] h-[285px] sm:border border-gray-200 rounded-xl ">
                <img
                  className="object-cover"
                  src="https://images.unsplash.com/photo-1489599849927-2ee91cede3ba?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxzZWFyY2h8MTd8fG1vdmllJTIwJTIwcG9zdGVyJTIwcGxhY2Vob2xkZXJ8ZW58MHx8MHx8&auto=format&fit=crop&w=800&q=60" alt="placeholder"
                />
              </div>
            )}
